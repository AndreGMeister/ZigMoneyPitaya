...
class Cliente extends Model
{
    ...
    public function clientesParaOpdv($idEmpresa)
    {
        return $this->query("
            SELECT id, nome, email, cnpj, cpf, telefone, celular FROM clientes WHERE id_empresa = {$idEmpresa} AND deleted_at IS NULL
        ");
    }
     ...
     */
    public function pesquisarClientesParaOpdv($termo, $idEmpresa)
    {
        $termo = out64($termo);
        return $this->queryGetOne("
            SELECT id, id_cliente_tipo, nome, email, cnpj, cpf, telefone, celular FROM clientes
            WHERE id_empresa = {$idEmpresa} AND deleted_at IS NULL
            AND
            nome LIKE '%{$termo}%'
            OR email LIKE '%{$termo}%'
            OR cnpj = '{$termo}'
